...
class D3Component {
    ...
    this.quadrants = [
      {
        ...
        startAngle: 90,
        name: "Techniques",
        items: data.filter((item) => item.quadrant === "Techniques"),
      },
      {
        order: "second",
        startAngle: 0,
        name: "Platforms",
        items: data.filter((item) => item.quadrant === "Platforms"),
      },
      {
        order: "third",
        startAngle: -90,
        name: "Tools",
        items: data.filter((item) => item.quadrant === "Tools"),
      },
      {
        order: "fourth",
        startAngle: -180,
        ...
        items: data.filter(
          (item) => item.quadrant === "languages-and-frameworks"
        ),
      },
    ];

    this.radarElement = d3
      .select(containerEl)
      .append("div")
      .attr("id", "radar");
  ...
  plotQuadrant = (rings, quadrant) => {
    ...
    var quadrantGroup = svg
      ...
      .on(
        "click",
        this.selectQuadrant.bind({}, quadrant.order, quadrant.startAngle)
      );
  ...
  plotBlips = (quadrantGroup, rings, quadrant) => {
    ...
    rings.forEach((ring, i) => {
      ...
      chance = new Chance(
        Math.PI * sumRing * ring.length * sumQuadrant * quadrant.name.length
      );
  ...
  plotRadarHeader = () => {
    ...
    header
      ...
      .html(
        '<a href="https://granulate.io"> <img src="/images/logo.png" /> </a>'
      );

    this.buttonsGroup = header.append("div").classed("buttons-group", true);

    this.quadrantButtons = this.buttonsGroup
      .append("div")
  ...
  drawBlipInCoordinates = (
    ...
    var blipText =
      blip.id + ". " + blip.name + (blip.topic ? ". - " + blip.topic : "");
    blipListItem
      .append("div")
